...
openFDA label fallback, ADR-0004), plus deterministic renal-dosing threshold checks (Phase 2 —
Cockcroft-Gault CrCl + 2021 race-free CKD-EPI eGFR, ADR-0005) — see `docs/ARCHITECTURE.md`
§Medication-Safety Engine. `pip_flags` and `discrepancies` are later phases, not yet implemented;
the response shape below reflects only what's actually built.
...
| POST | `/chats/{id}/medications/analyze` `{raw_text, age_years?, weight_kg?, sex?, serum_creatinine_mg_dl?, height_cm?}` | `202` + SSE — a `medication` event per parsed, RxNorm-normalized, already-persisted medication (`{id, raw_text, name, rxcui, dose, route, frequency}`), an `interaction` event per DDI finding (`{id, medication_a_id, medication_b_id, severity, source, rule_id, explanation, provenance}`), a `renal` event per renal-dosing threshold finding (`{id, medication_id, crcl_ml_min, egfr_ml_min, threshold_ml_min, severity, rule_id, explanation}` — only emitted when the five renal parameters are supplied; they're all-or-nothing, a partial set is a `422`), a final `done` event (`{status}`), or an `error` event (`provider_unavailable` \| `budget_exceeded`, degraded mode). Same SSE shape and stream-ticket bridge as `/verify`. `severity` ∈ `{major, moderate, minor, unclassified}`; `source` ∈ `{ddinter, openfda_label, calculated}`. |
